(function(){
  const K = {
    suppliers: "or_v130_suppliers",
    items: "or_v130_items",
    active: "or_v130_active",
    orders: "or_v130_orders",
    draft: "or_v130_draft",
    notes: "or_v130_notes",
    meta: "or_v130_meta" // branch per supplier
  };
  ...
  let suppliers = load(K.suppliers, DEFAULT_SUPPLIERS).map(s=>({ channels:["wa", ...(s.email?["email"]:[]), ...(s.channels||[])].filter((v,i,a)=>a.indexOf(v)===i), ...s }));
  ...
  // Supplier picker UI
  function buildSupplierPicker(){
    const btn=$("#supplierBtn"), label=$("#supplierBtnLabel"), menu=$("#supplierMenu");
    const s = suppliers.find(x=>x.phone===active);
    label.textContent = s ? s.name : "בחר ספק";
    menu.innerHTML = "";
    suppliers.forEach(sp=>{
      ...
      row.innerHTML=`<div>${sp.name}</div><div class="bnames">${bnames}</div>`;
      row.onclick=()=>{ active=sp.phone; save(K.active,active); menu.classList.remove("open"); buildOrders();};
  ...
  function buildOrders(){
    ...
    if(allowed.length<=1){
      ...
      allowed.forEach(bid=>{
        ...
        t.onclick=()=>{ meta[active] = {...(meta[active]||{}), branchId: bid}; save(K.meta,meta); $$(".seg-btn", seg).forEach(el=>el.classList.remove("active")); t.classList.add("active"); updatePreview(); };
        seg.appendChild(t);
      });
      bw.appendChild(seg);
    }
  ...
  $(".modal-backdrop").onclick=()=>modal.classList.remove("show");

  const waIcon = ()=>'<svg viewBox="0 0 24 24" fill="none" stroke="#25D366" stroke-width="2"><path d="M20.5 11.5a8.5 8.5 0 1 1-15.1 5L3 21l4.6-2.3a8.5 8.5 0 1 1 12.9-7.2Z"/></svg>';
  ...
  function renderSettings(){
    ...
    // Items editor for selected supplier (active or first)
    renderItemsEditor();
  }
  ...
  function renderItemsEditor(phone){
    ...
    arr.forEach((it,idx)=>{
      ...
      row.querySelector(".up").onclick = ()=>{ if(idx>0){ arr.splice(idx-1,0,arr.splice(idx,1)[0]); save(K.items,items); renderItemsEditor(p); buildOrders(); } };
      row.querySelector(".down").onclick = ()=>{ if(idx<arr.length-1){ arr.splice(idx+1,0,arr.splice(idx,1)[0]); save(K.items,items); renderItemsEditor(p); buildOrders(); } };
      row.querySelector(".del").onclick = ()=>{ arr.splice(idx,1); save(K.items,items); renderItemsEditor(p); buildOrders(); };
      ed.appendChild(row);
    });
